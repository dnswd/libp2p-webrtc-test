...
const a_key = await createFromJSON(akeys)
...
libp2p.addEventListener('peer:discovery', (evt) => {
    ...
    libp2p.dialProtocol(evt.detail.id, protocol).catch(err => {
        ...
        console.log(`nodeA dialed to nodeB on protocol: ${protocol}`)
        pipe(
            // Source data
...
await libp2p.handle('/echo/1.0.0', ({ stream }) => pipe(stream.source, stream.sink))
